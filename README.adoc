...
image::ultima-nexus-logo.png[Ultima Nexus JVM Check,200]
...
== Quick Start with Gradle Plugins (Recommended)

For projects using the https://github.com/daymor-dev/ultima-nexus-jvm-gradle-plugins[Ultima Nexus JVM Gradle Plugins]:
...
That's it! The plugins automatically resolve this artifact from Maven Central and apply all quality configurations. No additional configuration required.

=== Pinning a Specific Version (Optional)

To use a specific version instead of the plugin's built-in fallback, add to your `gradle/libs.versions.toml`:

[source,toml]
----
[versions]
ultima-nexus-jvm-check = "1.2.0"

[libraries]
ultima-nexus-jvm-check = { module = "dev.daymor.ultima-nexus.jvm:ultima-nexus-jvm-check", version.ref = "ultima-nexus-jvm-check" }
----

For detailed setup and configuration options, see:

* https://github.com/daymor-dev/ultima-nexus-jvm-gradle-plugins[Ultima Nexus JVM Gradle Plugins] - Full framework documentation
* https://github.com/daymor-dev/ultima-nexus-jvm-gradle-plugins/blob/main/plugins-check/README.adoc[plugins-check module] - Detailed check plugin configuration
...
tasks.withType<Pmd>().configureEach {
    ...
    // Use relaxed ruleset for test sources
    if (name != "pmdMain") {
        ruleSetConfig = resources.text.fromArchiveEntry(checkJarFile, "pmdRuleset-test.xml")
    }
}
...
* `pmdRuleset.xml` - PMD rules for main sources (all Java categories, strict)
* `pmdRuleset-test.xml` - PMD rules for test sources (relaxed, excludes false positives)
...
* https://github.com/daymor-dev/ultima-nexus-jvm-gradle-plugins[Ultima Nexus JVM Gradle Plugins]
* https://github.com/daymor-dev/ultima-nexus-jvm-gradle-plugins/blob/main/plugins-check/README.adoc[plugins-check Module Documentation]
